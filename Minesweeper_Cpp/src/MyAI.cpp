// ======================================================================
// FILE: MyAI.cpp   (see MyAI.hpp for the high-level strategy)
// ======================================================================
...
Agent::Action MyAI::getAction(int number)
{
    ...
    // 2. Inference / action loop.
    while (true)
    {
        while (deduce()) { /* iterate to fixpoint */ }
        ...
        pair<int,int> g = solveCSP(progressed);
        if (progressed)
...
bool MyAI::singlePoint()
{
    ...
    for (int x = 0; x < colDimension; x++)
        for (int y = 0; y < rowDimension; y++)
        {
            ...
            if (need == 0)
            {
                for (int i = 0; i < cc; i++) { markSafe(cx[i], cy[i]); changed = true; }
            }
            else if (need == covered)
            {
                for (int i = 0; i < cc; i++) { markMine(cx[i], cy[i]); changed = true; }
            }
        }
    return changed;
}
...
bool MyAI::subsetRule()
{
    ...
    for (int i = 0; i < C; i++)
        for (int id : vars[i]) byCell[id].push_back(i);
    ...
    for (int a = 0; a < C; a++)
    {
        ...
        for (int b : byCell[anchor])
        {
            ...
            {
                if (vars[a][ia] == vars[b][ib]) { ia++; ib++; }
                else if (vars[b][ib] < vars[a][ia]) ib++;
                else { sub = false; break; }
            }
            ...
            {
                if (ia < vars[a].size() && vars[a][ia] == vars[b][ib]) { ia++; ib++; continue; }
                int id = vars[b][ib];
                int x = id / rowDimension, y = id % rowDimension;
                if (board[id] == -2 && !safeMark[id])
                {
                    if (dneed == 0) { markSafe(x, y); changed = true; }
                    else            { markMine(x, y); changed = true; }
                }
                ib++;
            }
        }
                         ...
                         vector<vector<double>>& mineByK)
{
    ...
    for (int c = 0; c < C; c++)
        for (int v : conVars[c]) varCons[v].push_back(c);
    ...
    for (int c = 0; c < C; c++) conRem[c] = (int)conVars[c].size();
    ...
    struct Rec {
        ...
        {
            if (*overflow) return;
            if (++(*nodes) > nodeCap) { *overflow = true; return; }
            if (((*nodes) & 0x3FFFF) == 0 && nowSeconds() > deadline) { *overflow = true; return; }
                        ...
                        pair<int,int>& guess)
{
    ...
    for (int i = 0; i < n; i++) local[candidates[i]] = i;
    ...
    struct Rec {
        ...
        void go(int idx, int curMines)
        {
            if (*overflow) return;
            if (++(*nodes) > nodeCap) { *overflow = true; return; }
            if (((*nodes) & 0x3FFF) == 0 && nowSeconds() > deadline) { *overflow = true; return; }
            ...
            if (idx == n)
            {
                ...
                for (int v = 0; v < n; v++)
                    if ((*assign)[v]) mask |= (1ULL << v);
                layouts->push_back(mask);
                if (layouts->size() > layoutCap) *overflow = true;
                return;
            }
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int b = 0; b <= 8; b++) ambiguity += (double)buckets[b] * (double)buckets[b];
        if (p < bestP - 1e-12 ||
            (fabs(p - bestP) <= 1e-12 &&
...
pair<int,int> MyAI::solveCSP(bool& progressed)
{
    ...
    for (int id = 0; id < N; id++)
        if (board[id] == -2 && !safeMark[id]) { candidates.push_back(id); isCand[id] = 1; }
    ...
    if (R <= 0)
    {
        for (int id : candidates) markSafe(id / rowDimension, id % rowDimension);
        progressed = true; return { -1, -1 };
    }
    if (R >= (int)candidates.size())
    {
        for (int id : candidates) markMine(id / rowDimension, id % rowDimension);
        progressed = true; return { -1, -1 };
    }
    ...
    {
        if (progressed) return { -1, -1 };
        return exactGuess;
    }

    // Frontier and sea partition.
    vector<int> frontier, sea;
    for (int id : candidates) (isFront[id] ? frontier : sea).push_back(id);
    ...
    if (frontier.empty())
    {
        ...
        for (int id : sea)
        {
            ...
            if (nb < bestScore) { bestScore = nb; best = id; }
        }
    ...
    vector<int> parent(N);
    for (int id : frontier) parent[id] = id;
    ...
    {
        int r = find(id);
        if (rootIndex[r] == -1) { rootIndex[r] = (int)roots.size(); roots.push_back(r); compVars.push_back({}); }
    ...
    for (size_t ci = 0; ci < cons.size(); ci++)
    {
        ...
        vector<int> lv;
        for (int id : cons[ci]) lv.push_back(localIdx[id]);
    ...
    for (int c = 0; c < numComp; c++)
    {
        ...
        else
        {
            ...
            for (size_t k = 0; k < compCons[c].size(); k++)
            {
                ...
                for (int lv : compCons[c][k]) { heurP[c][lv] += frac; deg[lv]++; }
            }
            for (int j = 0; j < n; j++)
    ...
    bool allSolved = true;
    for (int c = 0; c < numComp; c++) if (!solved[c]) allSolved = false;
    ...
    if (allSolved)
    {
        ...
        if (seaSize > 0)
        {
            ...
            for (int m = 0; m <= seaSize; m++)
            {
                ...
                if (first || logBin[m] > maxLog) { maxLog = logBin[m]; first = false; }
            }
        }
        ...
        for (int T = 0; T < (int)total.size(); T++)
            if (total[T] != 0) Z += total[T] * binW(R - T);
        ...
        for (int c = 0; c < numComp; c++)
        {
            ...
            for (size_t i = 0; i < A.size(); i++) if (A[i] != 0)
                for (size_t j = 0; j < B.size(); j++) G[i + j] += A[i] * B[j];
            ...
            for (int j = 0; j < n; j++)
            {
                double num = 0.0;
                for (int k = 0; k <= n; k++)
                    if (compMine[c][k][j] != 0) num += compMine[c][k][j] * W[k];
    ...
    else
    {
        ...
        for (int c = 0; c < numComp; c++)
        {
            ...
            if (solved[c])
            {
                ...
                for (int k = 0; k < (int)compCnt[c].size(); k++) { tot += compCnt[c][k]; em += k * compCnt[c][k]; }
                if (tot <= 0) tot = 1.0;
                for (int j = 0; j < n; j++)
                {
                    double num = 0.0;
                    for (int k = 0; k < (int)compCnt[c].size(); k++) num += compMine[c][k][j];
    ...
    for (int id : frontier)
    {
        double p = probId[id];
        if (p < 0) continue;
        if (p <= EPS)        { markSafe(id / rowDimension, id % rowDimension); forced = true; }
        else if (p >= 1.0 - EPS) { markMine(id / rowDimension, id % rowDimension); forced = true; }
    }
    if (seaSize > 0)
    {
        if (seaProb <= EPS)       { for (int id : sea) markSafe(id / rowDimension, id % rowDimension); forced = true; }
        else if (seaProb >= 1.0 - EPS) { for (int id : sea) markMine(id / rowDimension, id % rowDimension); forced = true; }
    }
    if (forced) { progressed = true; return { -1, -1 }; }
    ...
    if (seaSize > 0 && seaProb >= 0)
    {
        ...
        int rep = -1, bestSeaScore = -1000000000;
        for (int id : sea)
        {
            int x = id / rowDimension, y = id % rowDimension;
            ...
            if (score > bestSeaScore) { bestSeaScore = score; rep = id; }
        }
